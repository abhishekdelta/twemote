...
parser.add_argument('-t','--authfile',metavar='FILE',default='.sendsms.auth',help='Specify the authorization file mannually. By default it is ~/.sendsms.auth')
...
if args.setup:
    ...
    config.set('Login','username',sys.argv[2])
    config.set('Login','password',sys.argv[3])
...
def login(uname,passwd):
    ...
    if f.read().find(confget('Auth','logindone')) != -1:
        ...
        return False
if (len(message)>160):
	message_parts = [message[i:i+150] for i in range(0,len(message),150)]
	numofpages = len(message_parts)
	pagenum = 0
		...
		for n in senders_numbers:
			...
			if not try_with_previous:
			 ### Logging in
				...
				sys.exit(3)
			msg = "Page "+str(pagenum)+" of "+str(numofpages)+"			"+msg
			result=sendmessage(n,msg)
			if result=='sent':
